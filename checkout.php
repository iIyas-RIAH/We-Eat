...
if(isset($_POST['submit'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, name, phone, num_table, total_products, total_price, placed_on) VALUES(?,?,?,?,?,?,?)");
      $insert_order->execute([$user_id, $name, $number, $table, $total_products, $total_price, date("Y-m-d H:i:sa")]);

      $update_profile = $conn->prepare("UPDATE `users` SET points = points + ? WHERE id = ?");
      $update_profile->execute([$total_price, $user_id]);
...
<form action="" method="post">
   ...
   <input type="hidden" name="number" value="<?= $fetch_profile['phone'] ?>">
